...
from supporting_files.soil_moisture_dynamics_lb_pap import SoilMoisture
from supporting_files.vegetation_dynamics_lb_pap import Vegetation
from supporting_files.plant_competition_ca_lb_pap import VegCA
...
def plot_results(grid, VegType, yrs, yr_step=10):
    ...
    plt.ylim(ymin=0, ymax=(max(np.max(grass_cov),
                               np.max(shrub_cov),
                               np.max(tree_cov))+10))
    plt.ylabel(' % Coverage ', weight='bold', fontsize=14)
    plt.xlabel('Time in years', weight='bold', fontsize=14)
    plt.legend(loc=0)
